...
void editorUpdateSyntax(erow *row)
{
    ...
    int i = 0;
    //goes through each character in the row and then highlight it
    while (i < row->rsize)
    {
        char c = row->render[i];
...
char *editorPrompt(char *prompt, void (*callback)(char *, int))
{
    ...
    while (1)
    {
        ...
        else if (c == '\x1b')
        {
            ...
            if (callback)
                callback(buf, c); //passes the key and the buffer. Functions use the key and determine what to do
            free(buf);
            return NULL;
        }
        else if (c == '\r')
